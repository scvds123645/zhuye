...
interface App {
  ...
  icon: string;
  url?: string;
  shareUrl?: string; // 已添加 shareUrl 字段
}
...
const SoftwareDownload = () => {
  ...
  const apps: App[] = [
    ...
    },
    {
      ...
      shareUrl: "https://www.584136.xyz/fb180" // 已更新分享链接
    },
    {
      id: "fb-accounts",
      name: "3万多个未180脸书账号",
  ...
  const handleShare = async (app: App) => {
    // 优先使用 app.shareUrl，如果不存在则使用动态生成的链接
    const url = app.shareUrl || `${window.location.origin}/rj?app=${encodeURIComponent(app.name)}`;
    if (navigator.share) {
      try {
